...
// ============================================================
// After deploying google-apps-script.js, paste your web app
...
function initDarkMode() {
  ...
  document.getElementById('dark-mode-toggle')?.addEventListener('click', () => {
    ...
    if (icon) icon.className = on ? 'fas fa-sun text-lg w-5 text-center' : 'fas fa-moon text-lg w-5 text-center';
    // Re-render charts with new theme
    if (_charts.weekly)  renderWeeklyChart(_lastOverview);
    if (_charts.trend)   renderTrendChart(_lastTrend);
    if (_charts.dca)     updateDCAChart();
  });
}
...
function setupQuickLog() {
  document.getElementById('quick-log-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      await api('logspend', { date: todayKey(), amount, bucket, desc: 'Manual entry (web)' });
...
function populateBucketDropdown(buckets) {
  ...
  sel.innerHTML = buckets.map(b =>
    `<option value="${b.id}"${b.id === 'daily_spending' ? ' selected' : ''}>${b.name}</option>`
  ).join('');
}
...
function renderSummaryCards(data) {
  const tk         = todayKey();
  const todayDay   = (data.days || []).find(d => d.date === tk);
  const todaySpent = todayDay?.spent || 0;
  const weekSpent  = (data.days || []).reduce((s, d) => s + (d.spent || 0), 0);
  const monthSpent = data.total_spent || 0;
  const monthAlloc = data.total_allocated || 1;

  setCard('card-today', todaySpent, DAILY_BUDGET, 'Today', `${data.days_left ?? '?'}d left this month`);
  setCard('card-week',  weekSpent,  DAILY_BUDGET * 7, 'This Week', '7-day rolling');
  setCard('card-month', monthSpent, monthAlloc, fmtMonth(_currentMonth), `${data.days_left ?? '?'} days left`);
}
        ...
        annotation: {
          annotations: {
            limit: {
              ...
              label: { content: fmt(DAILY_BUDGET), display: true, position: 'end', color: '#f59e0b', font: { size: 11 } },
            }
          }
        },
...
function renderBudgetBreakdown(data) {
  ...
  if (!buckets.length) { el.innerHTML = '<p class="text-sm text-gray-400 text-center py-4">No budget data</p>'; return; }
...
function renderNetWorth(data) {
  ...
  const loanRows = (data.loans_given || []).map(l =>
    `<tr class="border-b border-gray-100 dark:border-gray-800">
      ...
      <td class="py-2 text-xs text-gray-400">${l.annual_rate}%/yr · ${fmt(l.monthly_interest)}/mo interest due</td>
...
function updateDCAChart() {
  ...
  if (!ctx) return;
  if (_charts.dca) _charts.dca.destroy();
